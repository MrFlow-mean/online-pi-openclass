...
const CHECK_LABELS: Record<string, string> = {
  course_integrity: "课程历史完整性",
  source_access: "资料可访问性",
  publication_safety: "发布安全",
  export_readiness: "课程导出准备",
};
...
export function ProjectGovernancePanel({ projectKind, projectId }: { projectKind: ProjectKind; projectId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(failure instanceof Error ? failure.message : "项目治理信息载入失败");
    } finally {
      setLoading(false);
    }
  ...
  async function run(action: () => Promise<ProjectGovernance | void>, onSuccess?: () => void) {
    ...
    try {
      ...
      setError(failure instanceof Error ? failure.message : "项目协作操作失败");
    } finally {
      setBusy(false);
    }
  }
  ...
  const tabs = useMemo(
    () => [
      { id: "members" as const, label: "成员与权限", icon: Users },
      { id: "rules" as const, label: "审查与检查", icon: ShieldCheck },
      { id: "work" as const, label: "任务与看板", icon: KanbanSquare },
      { id: "integrations" as const, label: "组织与集成", icon: Network },
    ],
    []
  );

  if (loading) {
    return <div className="flex items-center justify-center gap-2 border-t border-stone-200 py-12 text-sm text-stone-500"><LoaderCircle className="h-5 w-5 animate-spin" />正在载入项目治理能力…</div>;
  }
  if (!governance) {
    return <div className="border-t border-stone-200 py-8 text-sm text-rose-700">{error ?? "无法载入项目治理能力。"}</div>;
  }

  return (
    <section className="mt-6 border-t border-stone-200 pt-5" aria-label={`${governance.title} 的项目治理`}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-stone-400">Repository governance</p>
          <h2 className="mt-1 text-lg font-semibold text-stone-950">项目治理</h2>
          <p className="mt-1 text-xs text-stone-500">当前角色：{ROLE_LABELS[governance.viewer_role]} · {governance.member_count} 位成员</p>
        </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {governance.lesson_ids.map((lessonId, index) => (
            <Link key={lessonId} href={`/profile/collaboration/${projectKind}/${encodeURIComponent(projectId)}/lesson/${encodeURIComponent(lessonId)}`} className="inline-flex items-center gap-2 rounded-full border border-stone-200 bg-white px-3 py-2 text-xs font-semibold text-stone-700 hover:border-stone-400">
              <PencilLine className="h-3.5 w-3.5" />{governance.capabilities.edit_project ? "协作编辑" : "查看课程"}{governance.lesson_ids.length > 1 ? ` ${index + 1}` : ""}
            </Link>
          ))}
        </div>
      ) : null}
        ...
        <div className="mt-4 space-y-4">
          {governance.capabilities.manage_members ? (
            <form onSubmit={(event) => { event.preventDefault(); if (!inviteEmail.trim()) return; void run(() => projectCollaborationApi.invite(projectKind, projectId, inviteEmail, inviteRole), () => setInviteEmail("")); }} className="grid gap-2 rounded-lg border border-stone-200 bg-stone-50 p-3 sm:grid-cols-[1fr_150px_auto]">
              <input type="email" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} placeholder="协作者邮箱" className="h-10 rounded-md border border-stone-200 bg-white px-3 text-sm outline-none focus:border-violet-400" />
              ...
              <button disabled={busy || !inviteEmail.trim()} className="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-stone-950 px-4 text-sm font-semibold text-white disabled:opacity-40"><UserPlus className="h-4 w-4" />邀请</button>
            </form>
          ) : null}
          <div className="divide-y divide-stone-200 rounded-lg border border-stone-200 bg-white">
            {governance.members.map((member) => (
              <div key={member.user_id} className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:justify-between">
                <div><p className="text-sm font-semibold text-stone-900">{member.display_name || member.email || "项目成员"}</p><p className="mt-1 text-xs text-stone-400">{member.email || member.user_id}</p></div>
      ...
      {activeTab === "rules" ? (
        <div className="mt-4 space-y-4">
          <div className="rounded-lg border border-stone-200 bg-white p-4">
            <div className="flex items-center gap-2"><GitBranch className="h-4 w-4 text-violet-600" /><h3 className="text-sm font-semibold">默认分支与合并规则</h3></div>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={governance.policy.protect_default_branch} disabled={!governance.capabilities.manage_rules} onChange={(event) => setGovernance({ ...governance, policy: { ...governance.policy, protect_default_branch: event.target.checked } })} />保护默认分支</label>
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={governance.policy.require_passing_checks} disabled={!governance.capabilities.manage_rules} onChange={(event) => setGovernance({ ...governance, policy: { ...governance.policy, require_passing_checks: event.target.checked } })} />合并前检查必须通过</label>
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={governance.policy.dismiss_stale_approvals} disabled={!governance.capabilities.manage_rules} onChange={(event) => setGovernance({ ...governance, policy: { ...governance.policy, dismiss_stale_approvals: event.target.checked } })} />新修订使旧批准失效</label>
              <label className="flex items-center gap-2 text-sm">所需批准数<input type="number" min={0} max={20} value={governance.policy.required_approvals} disabled={!governance.capabilities.manage_rules} onChange={(event) => setGovernance({ ...governance, policy: { ...governance.policy, required_approvals: Number(event.target.value) } })} className="h-9 w-20 rounded-md border border-stone-200 px-2" /></label>
            </div>
            {governance.capabilities.manage_rules ? <button type="button" disabled={busy} onClick={() => void run(() => projectCollaborationApi.updatePolicy(projectKind, projectId, governance.policy))} className="mt-4 rounded-md bg-stone-950 px-4 py-2 text-xs font-semibold text-white">保存规则</button> : null}
          </div>
          <div className="rounded-lg border border-stone-200 bg-white p-4">
            <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><ClipboardCheck className="h-4 w-4 text-emerald-600" /><h3 className="text-sm font-semibold">课程检查</h3></div><button type="button" disabled={busy || !governance.capabilities.review_changes} onClick={() => void run(async () => { setChecks(await projectCollaborationApi.runChecks(projectKind, projectId)); })} className="rounded-md border border-stone-200 px-3 py-2 text-xs font-semibold">重新检查</button></div>
      ...
      {activeTab === "work" ? (
        <div className="mt-4 space-y-4">
          {governance.capabilities.manage_work_items ? <form onSubmit={(event) => { event.preventDefault(); if (!workTitle.trim()) return; void run(() => projectCollaborationApi.createWorkItem(projectKind, projectId, workTitle, workBody, workMilestoneId, workLabelId ? [workLabelId] : []), () => { setWorkTitle(""); setWorkBody(""); setWorkMilestoneId(""); setWorkLabelId(""); }); }} className="rounded-lg border border-stone-200 bg-stone-50 p-3"><input value={workTitle} onChange={(event) => setWorkTitle(event.target.value)} placeholder="新任务标题" className="h-10 w-full rounded-md border border-stone-200 bg-white px-3 text-sm" /><textarea value={workBody} onChange={(event) => setWorkBody(event.target.value)} placeholder="任务说明" rows={2} className="mt-2 w-full rounded-md border border-stone-200 bg-white px-3 py-2 text-sm" /><div className="mt-2 grid gap-2 sm:grid-cols-2"><select value={workMilestoneId} onChange={(event) => setWorkMilestoneId(event.target.value)} className="h-9 rounded border border-stone-200 bg-white px-2 text-xs"><option value="">不关联里程碑</option>{governance.milestones.filter((item) => !item.closed).map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select><select value={workLabelId} onChange={(event) => setWorkLabelId(event.target.value)} className="h-9 rounded border border-stone-200 bg-white px-2 text-xs"><option value="">不添加标签</option>{governance.labels.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div><button disabled={busy || !workTitle.trim()} className="mt-2 inline-flex items-center gap-2 rounded-md bg-stone-950 px-4 py-2 text-xs font-semibold text-white"><Plus className="h-4 w-4" />创建任务</button></form> : null}
