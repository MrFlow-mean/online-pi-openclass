...
class OpenNotebookAdapter:
    """Small compatibility adapter for the Open Notebook REST API."""
    ...
    def __init__(
        ...
        *,
        ...
        self.api_url = (
            api_url or os.getenv("OPEN_NOTEBOOK_API_URL") or "http://localhost:5055"
        ).rstrip("/")
        self.password = (
            password
    ...
    def create_notebook(self, *, title: str, description: str = "") -> str:
        ...
        if not notebook_id:
            raise OpenNotebookAdapterError(
                "Open Notebook did not return a notebook id."
            )
        return notebook_id

    def add_url_source(
        self, *, notebook_id: str, source_uri: str, title: str = ""
    ...
    def get_command(self, command_id: str) -> dict[str, Any]:
        ...
        for path in (f"/commands/jobs/{command_id}", f"/commands/{command_id}"):
            try:
                ...
                last_error = exc
        raise last_error or OpenNotebookAdapterError(
            "Open Notebook command lookup failed."
        )
        ...
        for url in self._candidate_urls(path):
            try:
                ...
                if response.status_code == 404 and self.api_prefix:
                    last_error = OpenNotebookAdapterError(
                        f"Open Notebook returned 404 for {url}."
                    )
                    continue
                ...
                last_error = exc
        raise OpenNotebookAdapterError(
            str(last_error or "Open Notebook request failed.")
        )
...
def _source_result(data: dict[str, Any]) -> OpenNotebookSourceResult:
    ...
    status = _first_text(data, "status", "processing_status") or (
        "queued" if command_id else "ready"
    )
    if not source_id:
